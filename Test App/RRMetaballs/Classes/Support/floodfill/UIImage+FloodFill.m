...
- (UIImage *)floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor pixelColorBlock:(void (^)(int x, int y, int *r, int *g, int *b, int *a))pixelColorBlock {
    ...
    unsigned int bytesPerPixel = CGImageGetBitsPerPixel(imageRef) /8;
    unsigned int bytesPerRow   = CGImageGetBytesPerRow(imageRef);
    unsigned int bitsPerComponent = CGImageGetBitsPerComponent(imageRef);
    ...
    NSLog(@"%@ %u -> %u", NSStringFromCGPoint(startPoint), ocolor, ncolor);
    ...
    while ( [points popFront:&x andY:&y] != INVALID_NODE_CONTENT ){
        ...
        while( y >= 0 && compareColor(ocolor, color) ){
            y--;
        ...
        if( y >= 0 && !compareColor(ocolor, color) ) {
        ...
        while ( y < height && compareColor(ocolor, color) && !compareColor(ncolor, color) ) {
            int pxColorRed   = newRed;
            int pxColorGreen = newGreen;
            int pxColorBlue  = newBlue;
            int pxColorAlpha = newAlpha;
            ...
            imageData[byteIndex + 0] = MIN(pxColorRed,   255);
            imageData[byteIndex + 1] = MIN(pxColorGreen, 255);
            imageData[byteIndex + 2] = MIN(pxColorBlue,  255);
            imageData[byteIndex + 3] = MIN(pxColorAlpha, 255);
            ...
            if( x > 0 ) {
                ...
                if( !spanLeft && compareColor(ocolor, color) ) {
                    [points pushFrontX:(x -1) andY:y];
                    
                    spanLeft = YES;
                } else if( spanLeft && !compareColor(ocolor, color) ) {
                    spanLeft = NO;
                }
                
                // we can't go left. Add the point on the antialiasing list
                if( !spanLeft && !compareColor(ocolor, color) && !compareColor(ncolor, color) ) {
                    // point color is not same as start color
                    // point color is not same as new   color

//                    int red1   = ((0xff000000 & color) >> 24);
//                    int green1 = ((0x00ff0000 & color) >> 16);
//                    int blue1  = ((0x0000ff00 & color) >> 8);
...
//                    }
            ...
            if( x < width -1 ) {
                ...
                if( !spanRight && compareColor(ocolor, color) ) {
                    [points pushFrontX:(x +1) andY:y];
                    
                    spanRight = YES;
                } else if( spanRight && !compareColor(ocolor, color) ) {
                    spanRight = NO;
                }
                
                // we can't go right. Add the point on the antialiasing list
                if( !spanRight && !compareColor(ocolor, color) && !compareColor(ncolor, color) ) {
                    // point color is not same as start color
                    // point color is not same as new   color
 ...
 */
bool compareColor( unsigned int color1, unsigned int color2 ) {
    ...
    if( diffRed > 0.0f || diffGreen > 0.0f || diffBlue > 0.0f ) {
        return false;
    }
